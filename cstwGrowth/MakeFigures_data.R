...
panel_c <- ggplot(wealth %>% filter((source=="OECD WDD" | source=="Credit Suisse") &
  ...
  geom_text_repel(aes(label=iso), show.legend = FALSE, size = 2, segment.size = 0.2,
                  segment.alpha = 0.5, point.padding = 0.15) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE, size = 0.5)  +
  labs(x = "Growth rate", y = "Inequality", title = "(c) Top 10% relative to median") +
  theme_light() + 
  theme(plot.title = element_text(hjust = 0.5, size = 10),
        axis.title.x = element_text(size = 9),
        axis.title.y = element_text(size = 9))
...
# ------------------------------------------------------------------------------------ #
# Make figure with wealth shares
# ------------------------------------------------------------------------------------ #
panel_a <- ggplot(wealth %>% filter((source=="OECD WDD" | source=="Credit Suisse") &
                                      (quality=="Good" | quality=="Satisfactory") & iso!="CN"),
                  aes(x = growth_last20-1, y = top1_share, color = source)) +
  geom_point(size = 1) +
  geom_text_repel(aes(label=iso), show.legend = FALSE, size = 2, segment.size = 0.2,
                  segment.alpha = 0.5, point.padding = 0.15) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE, size = 0.5)  +
  labs(x = "Growth rate", y = "", title = "(a) Wealth share of the top 1%", color = "") +
  theme_light()  + 
  theme(plot.title = element_text(hjust = 0.5, size = 10),
        axis.title.x = element_text(size = 9),
        axis.title.y = element_text(size = 9),
...
panel_b <- ggplot(wealth %>% filter((source=="OECD WDD" | source=="Credit Suisse") &
                                      (quality=="Good" | quality=="Satisfactory") & iso!="CN"),
                  aes(x = growth_last20-1, y = top5_share, color = source)) +
  geom_point(size = 1) +
  geom_text_repel(aes(label=iso), show.legend = FALSE, size = 2, segment.size = 0.2,
                  segment.alpha = 0.5, point.padding = 0.15) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE, size = 0.5)  +
  labs(x = "Growth rate", y = "", title = "(b) Wealth share of the top 5%") +
  theme_light() + 
  theme(plot.title = element_text(hjust = 0.5, size = 10),
        axis.title.x = element_text(size = 9),
        axis.title.y = element_text(size = 9))
...
panel_c <- ggplot(wealth %>% filter((source=="OECD WDD" | source=="Credit Suisse") &
                                      (quality=="Good" | quality=="Satisfactory") & iso!="CN"),
                  aes(x = growth_last20-1, y = top10_share, color = source)) +
  geom_point(size = 1) +
  geom_text_repel(aes(label=iso), show.legend = FALSE, size = 2, segment.size = 0.2,
                  segment.alpha = 0.5, point.padding = 0.15) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE, size = 0.5)  +
  labs(x = "Growth rate", y = "", title = "(c) Wealth share of the top 10%") +
  theme_light() + 
  theme(plot.title = element_text(hjust = 0.5, size = 10),
        axis.title.x = element_text(size = 9),
        axis.title.y = element_text(size = 9))
...
panel_d <- ggplot(wealth %>% filter((source=="OECD WDD" | source=="Credit Suisse") &
                                      (quality=="Good" | quality=="Satisfactory") & iso!="CN"),
                  aes(x = growth_last20-1, y = bot60_share, color = source)) +
  geom_point(size = 1) +
  geom_text_repel(aes(label=iso), show.legend = FALSE, size = 2, segment.size = 0.2,
                  segment.alpha = 0.5, point.padding = 0.15) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE, size = 0.5)  +
  labs(x = "Growth rate", y = "", title = "(d) Wealth share of the bottom 60%") +
  theme_light() + 
  theme(plot.title = element_text(hjust = 0.5, size = 10),
        axis.title.x = element_text(size = 9),
        axis.title.y = element_text(size = 9))

shares <- grid.arrange(arrangeGrob(panel_a + theme(legend.position="none", plot.margin = unit(c(0.3,0.3,0.3,0.3), "cm")),
                                             panel_b + theme(legend.position="none", plot.margin = unit(c(0.3,0.3,0.3,0.3), "cm")),
                                             panel_c + theme(legend.position="none", plot.margin = unit(c(0.3,0.3,0.3,0.3), "cm")),
                                             panel_d + theme(legend.position="none", plot.margin = unit(c(0.3,0.3,0.3,0.3), "cm")),
                                             nrow = 2),
                                 g_legend(panel_a), nrow = 2, heights=c(10,1))
ggsave("shares.pdf", plot = shares, device = "pdf", width = 8, height = 8)
ggsave("../../tex/shares.pdf", plot = shares, device = "pdf", width = 8, height = 8)


#-----------------------------------------------#
# Plot mean to median over last 30 and 40 years
#-----------------------------------------------#

panel_a <- ggplot(wealth %>% filter((source=="OECD WDD" | source=="Credit Suisse") &
                           (quality=="Good" | quality=="Satisfactory") & iso!="CN"),
       aes(x = growth_last30-1, y = mean_to_median, color = source)) +
  geom_point(size = 1) +
  geom_text_repel(aes(label=iso), show.legend = FALSE, size = 2, segment.size = 0.2,
                  segment.alpha = 0.5, point.padding = 0.15) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE, size = 0.5)  +
...
panel_b <- ggplot(wealth %>% filter((source=="OECD WDD" | source=="Credit Suisse") &
                                      (quality=="Good" | quality=="Satisfactory") & iso!="CN"),
                  aes(x = growth_last40-1, y = mean_to_median, color = source)) +
  geom_point(size = 1) +
  geom_text_repel(aes(label=iso), show.legend = FALSE, size = 2, segment.size = 0.2,
                  segment.alpha = 0.5, point.padding = 0.15) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE, size = 0.5)  +
...
m2m_30_40 <- grid.arrange(arrangeGrob(panel_a + theme(legend.position="none", plot.margin = unit(c(0.3,0.3,0.3,0.3), "cm")),
                                      panel_b + theme(legend.position="none", plot.margin = unit(c(0.3,0.3,0.3,0.3), "cm")),
                                      nrow = 1),
                          g_legend(panel_a), nrow = 2, heights=c(10,1))
ggsave("m2m_30_40.pdf", plot = m2m_30_40, device = "pdf", width = 8, height = 4)
ggsave("../../tex/m2m_30_40.pdf", plot = m2m_30_40, device = "pdf", width = 8, height = 4)
