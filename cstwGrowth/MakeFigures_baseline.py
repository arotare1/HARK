'''
Creates afigures for baseline calibration and saves them in ./Figures/Baseline/
...
# Plot average Lorenz curves for wealth levels
...
for j in range(len(growthFactors)):
    plt.plot(LorenzAxis, LorenzLongLvlSim[j], '--', color=next(colors),
             label='g=' + str(annual_growthFactors[j]))
